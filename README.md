...
| `GET /api/tasks/{id}/runs` | 执行日志(最近 30 条,含耗时与完整生成内容) |
| `POST /api/tasks/test-dingtalk` | 发送钉钉测试消息(webhook, secret) |
| `GET /api/conversations` | 搜索历史列表(最近 100 条) |
| `GET /api/conversations/{id}` | 会话详情(完整消息) |
| `POST /api/conversations` | 保存/更新会话(带 id 即更新) |
